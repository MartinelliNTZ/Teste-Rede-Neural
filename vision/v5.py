# -*- coding: utf-8 -*-
"""
Pipeline de Classificação: Random Forest + Vetorização
======================================================
Entrada : 1 TIFF RGB+Alpha  +  shapefiles de polígonos de treino
Saída   : TIFF classificado + GeoJSON/Shapefile por classe (QGIS-ready)

Uso
  python main4.py           # treina + prediz + vetoriza
  python main4.py --retrain # força re-treino do modelo

Dependências
  pip install rasterio fiona shapely pyproj scikit-learn scipy joblib numpy scikit-image
"""

import os
import sys
import glob
import json
import warnings
import numpy as np
import rasterio
from rasterio.features import rasterize, shapes as rio_shapes
from rasterio.windows import Window
import fiona
from pyproj import CRS, Transformer
from shapely.geometry import shape, mapping
from shapely.ops import transform as shp_transform
from scipy.ndimage import binary_opening, binary_closing, uniform_filter, label as nd_label, maximum_filter
from sklearn.ensemble import RandomForestClassifier
from sklearn.model_selection import train_test_split
from sklearn.metrics import classification_report, accuracy_score
import joblib

warnings.filterwarnings("ignore")

# =============================================================================
#  ██████╗ ██████╗ ███╗   ██╗███████╗██╗ ██████╗ ██╗   ██╗██████╗  █████╗ ██████╗
# ██╔════╝██╔═══██╗████╗  ██║██╔════╝██║██╔════╝ ██║   ██║██╔══██╗██╔══██╗██╔══██╗
# ██║     ██║   ██║██╔██╗ ██║█████╗  ██║██║  ███╗██║   ██║██████╔╝███████║██████╔╝
# ██║     ██║   ██║██║╚██╗██║██╔══╝  ██║██║   ██║██║   ██║██╔══██╗██╔══██║██╔══██╗
# ╚██████╗╚██████╔╝██║ ╚████║██║     ██║╚██████╔╝╚██████╔╝██║  ██║██║  ██║██║  ██║
#  ╚═════╝ ╚═════╝ ╚═╝  ╚═══╝╚═╝     ╚═╝ ╚═════╝  ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝
#
#  Edite APENAS este bloco para configurar o pipeline completo
# =============================================================================

# ── Caminhos ─────────────────────────────────────────────────────────────────
ROOT     = os.path.abspath(".")
DATA_DIR = os.path.join(ROOT, "dados")
OUT_DIR  = os.path.join(ROOT, "saida")

# ── Classes ───────────────────────────────────────────────────────────────────
# Formato: id_numerico: { "shp": caminho, "nome": legenda, "cor_qgis": RGB }
# Para adicionar/remover classes: edite apenas este dicionário.
# A classe "outros" (id=4) é gerada automaticamente e NÃO precisa de shapefile.
CLASSES = {
    1: {
        "shp":       os.path.join(DATA_DIR, "palhada.shp"),
        "nome":      "palhada",
        "cor_qgis":  (255, 230, 100),   # amarelo
    },
    2: {
        "shp":       os.path.join(DATA_DIR, "solo.shp"),
        "nome":      "solo",
        "cor_qgis":  (180, 120,  60),   # marrom
    },
    3: {
        "shp":       os.path.join(DATA_DIR, "floresta.shp"),
        "nome":      "floresta",
        "cor_qgis":  ( 30, 160,  60),   # verde
    },
    # Exemplo para adicionar uma 4ª classe real:
    # 4: {
    #     "shp":      os.path.join(DATA_DIR, "agua.shp"),
    #     "nome":     "agua",
    #     "cor_qgis": (60, 120, 255),
    # },
}

# Classe "outros" — gerada automaticamente para pixels de baixa confiança
CLASSE_OUTROS_ID   = 99          # id usado internamente (não conflite com CLASSES)
CLASSE_OUTROS_NOME = "outros"

# ── Treino ────────────────────────────────────────────────────────────────────
SAMPLES_PER_CLASS  = 60_000   # pixels de treino por classe (balanceado)
VALIDATION_SPLIT   = 0.20     # fração reservada para relatório de acurácia
RF_N_TREES         = 200      # árvores — mais = mais lento, mais preciso
RF_JOBS            = -1       # -1 = usa todos os núcleos da CPU

# ── Predição ──────────────────────────────────────────────────────────────────
TILE_SZ            = 2048     # pixels por tile (ajuste conforme RAM disponível)
CONF_THRESHOLD     = 0.45     # confiança mínima; abaixo → "outros"

# ── Vetorização ───────────────────────────────────────────────────────────────
MIN_AREA_M2        = 3.0      # área mínima de polígono E de buraco (m²)
                               # polígonos/buracos menores são AGREGADOS ao vizinho
SMOOTH_ITER        = 2        # iterações de abertura/fechamento morfológico

# =============================================================================
#  FIM DAS CONFIGURAÇÕES — não é necessário editar abaixo desta linha
# =============================================================================

# ── Derivados ─────────────────────────────────────────────────────────────────
MODEL_PATH  = os.path.join(OUT_DIR, "model_rf.joblib")
CLASS_NAMES = {cid: cfg["nome"] for cid, cfg in CLASSES.items()}
CLASS_NAMES[CLASSE_OUTROS_ID] = CLASSE_OUTROS_NOME

os.makedirs(OUT_DIR, exist_ok=True)


# ─────────────────────────────────────────────────────────────────────────────
# UTILITÁRIOS
# ─────────────────────────────────────────────────────────────────────────────

def find_tiff():
    tiffs = (glob.glob(os.path.join(DATA_DIR, "*.tif")) +
             glob.glob(os.path.join(DATA_DIR, "*.tiff")))
    if not tiffs:
        raise FileNotFoundError(f"Nenhum TIFF encontrado em: {DATA_DIR}")
    return tiffs[0]

def tiff_info(path):
    with rasterio.open(path) as src:
        return src.meta.copy(), src.crs, src.transform, src.height, src.width

def reproject_geom(geom, src_crs, dst_crs):
    t = Transformer.from_crs(src_crs, dst_crs, always_xy=True)
    return shp_transform(t.transform, geom)

def load_geoms(shp_path, tiff_crs):
    geoms = []
    with fiona.open(shp_path) as f:
        shp_crs = CRS.from_user_input(f.crs)
        reproj  = not shp_crs.equals(tiff_crs, ignore_axis_order=True)
        for feat in f:
            raw = feat.get("geometry")
            if raw is None:
                continue
            try:
                g = shape(raw).buffer(0)
            except Exception:
                continue
            if g.is_empty or not g.is_valid:
                continue
            if reproj:
                g = reproject_geom(g, shp_crs, tiff_crs)
            geoms.append(g)
    return geoms


# ─────────────────────────────────────────────────────────────────────────────
# FEATURES ESPECTRAIS  (18 features)
# ─────────────────────────────────────────────────────────────────────────────

FEAT_NAMES = [
    "R", "G", "B",
    "ExG",   # Excess Green         = 2G − R − B   (vegetação)
    "ExR",   # Excess Red           = 1.4R − G      (solo)
    "ExGR",  # ExG − ExR            (contraste veg×solo)
    "VARI",  # (G−R)/(G+R−B)        (dossel verde)
    "NGRDI", # (G−R)/(G+R)          (palhada×floresta)
    "GLI",   # (2G−R−B)/(2G+R+B)   (índice de folha verde)
    "MGRVI", # (G²−R²)/(G²+R²)     (vigor vegetativo)
    "brilho",
    "R/bri",
    "G/bri",
    "Sat",
    "Value",
    "stdR",  # textura local R
    "stdG",  # textura local G
    "stdB",  # textura local B
]

def compute_features(rgb_tile):
    """
    Entrada : float32 [H, W, 3]  valores 0–1
    Saída   : float32 [H, W, 18]
    """
    R, G, B = rgb_tile[..., 0], rgb_tile[..., 1], rgb_tile[..., 2]
    eps = 1e-6

    ExG   = 2*G - R - B
    ExR   = 1.4*R - G
    ExGR  = ExG - ExR

    denom_vari = G + R - B
    VARI  = np.where(np.abs(denom_vari) > 0.02, (G - R) / (denom_vari + eps), 0.0)
    NGRDI = np.where((G + R) > 0.02, (G - R) / (G + R + eps), 0.0)

    denom_gli  = 2*G + R + B
    GLI   = np.where(denom_gli > 0.02, (2*G - R - B) / (denom_gli + eps), 0.0)

    denom_mgrvi = G**2 + R**2
    MGRVI = np.where(denom_mgrvi > 1e-4, (G**2 - R**2) / (denom_mgrvi + eps), 0.0)

    bri   = (R + G + B) / 3.0 + eps
    r_r   = R / bri
    r_g   = G / bri
    Cmax  = np.maximum.reduce([R, G, B])
    Cmin  = np.minimum.reduce([R, G, B])
    S     = np.where(Cmax > 0.02, (Cmax - Cmin) / (Cmax + eps), 0.0)

    def local_std(ch):
        m1 = uniform_filter(ch.astype(np.float32), size=5)
        m2 = uniform_filter(ch.astype(np.float32)**2, size=5)
        return np.sqrt(np.maximum(m2 - m1**2, 0.0))

    feats = np.stack([
        R, G, B,
        ExG, ExR, ExGR,
        VARI, NGRDI, GLI, MGRVI,
        bri - eps, r_r, r_g,
        S, Cmax,
        local_std(R), local_std(G), local_std(B),
    ], axis=-1).astype(np.float32)

    return feats  # [H, W, 18]


# ─────────────────────────────────────────────────────────────────────────────
# ETAPA 1 — EXTRAÇÃO DE AMOSTRAS + TREINO
# ─────────────────────────────────────────────────────────────────────────────

def train(image_path):
    print("─" * 60)
    print("ETAPA 1: EXTRAÇÃO DE AMOSTRAS DE TREINO")
    print("─" * 60)

    meta, tiff_crs, transform, H, W = tiff_info(image_path)
    res_m = abs(transform.a)
    print(f"  TIFF : {H:,} × {W:,} px  |  resolução ≈ {res_m*100:.1f} cm/px")

    # ── Rasteriza todos os shapefiles ────────────────────────────────────────
    label_raster = np.zeros((H, W), dtype=np.uint8)
    for cid, cfg in CLASSES.items():
        cname    = cfg["nome"]
        shp_path = cfg["shp"]
        if not os.path.exists(shp_path):
            print(f"  [AVISO] {shp_path} não encontrado, pulando.")
            continue
        geoms = load_geoms(shp_path, tiff_crs)
        if not geoms:
            print(f"  [AVISO] Nenhuma geometria válida em {cname}.")
            continue
        burned = rasterize(
            [(g, cid) for g in geoms],
            out_shape=(H, W),
            transform=transform,
            fill=0,
            dtype=np.uint8,
        )
        label_raster = np.where(burned > 0, burned, label_raster)
        px = int((burned > 0).sum())
        print(f"  {cname}: {px:,} px disponíveis  ({100*px/(H*W):.2f}% da imagem)")

    # ── Extrai features por classe ────────────────────────────────────────────
    X_all, y_all = [], []
    with rasterio.open(image_path) as src:
        for cid, cfg in CLASSES.items():
            cname      = cfg["nome"]
            ys_c, xs_c = np.where(label_raster == cid)
            if len(ys_c) == 0:
                print(f"  [AVISO] {cname}: sem pixels após rasterização.")
                continue

            n   = min(SAMPLES_PER_CLASS, len(ys_c))
            idx = np.random.default_rng(42).choice(len(ys_c), n, replace=False)
            ys_s, xs_s = ys_c[idx], xs_c[idx]

            tile_y = ys_s // TILE_SZ
            tile_x = xs_s // TILE_SZ
            feats_list = []

            for ty in np.unique(tile_y):
                for tx in np.unique(tile_x[(tile_y == ty)]):
                    sel = (tile_y == ty) & (tile_x == tx)
                    y0  = int(ty * TILE_SZ)
                    x0  = int(tx * TILE_SZ)
                    h   = min(TILE_SZ, H - y0)
                    w   = min(TILE_SZ, W - x0)
                    win = Window(x0, y0, w, h)
                    raw = src.read([1, 2, 3], window=win).astype(np.float32) / 255.0
                    rgb = np.moveaxis(raw, 0, -1)
                    ft  = compute_features(rgb)
                    ly  = np.clip(ys_s[sel] - y0, 0, h - 1)
                    lx  = np.clip(xs_s[sel] - x0, 0, w - 1)
                    feats_list.append(ft[ly, lx])

            if not feats_list:
                continue
            X_c = np.vstack(feats_list)
            y_c = np.full(len(X_c), cid, dtype=np.int32)
            X_all.append(X_c)
            y_all.append(y_c)
            print(f"  {cname}: {len(X_c):,} amostras extraídas ✓")

    if not X_all:
        raise RuntimeError("Nenhuma amostra de treino extraída. Verifique os shapefiles.")

    X = np.vstack(X_all)
    y = np.concatenate(y_all)
    print(f"\n  Total: {len(X):,} amostras | {len(np.unique(y))} classes")

    # ── Split treino / validação ──────────────────────────────────────────────
    X_tr, X_val, y_tr, y_val = train_test_split(
        X, y,
        test_size=VALIDATION_SPLIT,
        stratify=y,
        random_state=42,
    )
    print(f"  Split: {len(X_tr):,} treino  /  {len(X_val):,} validação ({int(VALIDATION_SPLIT*100)}%)")

    # ── Treino ────────────────────────────────────────────────────────────────
    print("\n─" * 60)
    print("ETAPA 2: TREINO DO RANDOM FOREST")
    print("─" * 60)

    clf = RandomForestClassifier(
        n_estimators=RF_N_TREES,
        max_features="sqrt",
        min_samples_leaf=5,
        class_weight="balanced",
        oob_score=True,          # estimativa OOB sem custo extra
        n_jobs=RF_JOBS,
        verbose=0,
        random_state=42,
    )
    print(f"  Treinando {RF_N_TREES} árvores com {len(X_tr):,} amostras …")
    clf.fit(X_tr, y_tr)

    # ── Relatório de precisão ─────────────────────────────────────────────────
    print("\n─" * 60)
    print("RELATÓRIO DE PRECISÃO")
    print("─" * 60)

    oob = clf.oob_score_
    print(f"  OOB Score (estimativa interna RF) : {oob*100:.2f}%")

    y_pred_val = clf.predict(X_val)
    acc_val    = accuracy_score(y_val, y_pred_val)
    print(f"  Acurácia no conjunto de validação : {acc_val*100:.2f}%")

    target_names = [CLASS_NAMES[cid] for cid in sorted(np.unique(y_val))]
    print("\n  Relatório por classe:")
    report = classification_report(
        y_val, y_pred_val,
        target_names=target_names,
        digits=3,
    )
    for line in report.splitlines():
        print(f"    {line}")

    # ── Top features ──────────────────────────────────────────────────────────
    imp = clf.feature_importances_
    top = np.argsort(imp)[::-1][:7]
    print("\n  Top-7 features:")
    for i in top:
        print(f"    {FEAT_NAMES[i]:8s}: {imp[i]:.3f}")

    joblib.dump(clf, MODEL_PATH)
    print(f"\n  Modelo salvo: {MODEL_PATH}")
    return clf


# ─────────────────────────────────────────────────────────────────────────────
# ETAPA 2 — PREDIÇÃO
# ─────────────────────────────────────────────────────────────────────────────

def predict(clf, image_path):
    print("\n─" * 60)
    print("ETAPA 3: PREDIÇÃO EM TODA A IMAGEM")
    print("─" * 60)

    meta, _, transform, H, W = tiff_info(image_path)

    out_meta = {k: v for k, v in meta.items()
                if k not in ("count", "dtype", "nodata")}
    out_meta.update({"count": 1, "dtype": "uint8", "nodata": 0})

    pred_path   = os.path.join(OUT_DIR, "classificado.tif")
    n_ty        = (H + TILE_SZ - 1) // TILE_SZ
    n_tx        = (W + TILE_SZ - 1) // TILE_SZ
    total_tiles = n_ty * n_tx
    done        = 0

    with rasterio.open(image_path) as src:
        has_alpha = src.count >= 4
        with rasterio.open(pred_path, "w", **out_meta) as dst:
            for ty in range(n_ty):
                y0 = ty * TILE_SZ
                th = min(TILE_SZ, H - y0)
                for tx in range(n_tx):
                    x0  = tx * TILE_SZ
                    tw  = min(TILE_SZ, W - x0)
                    win = Window(x0, y0, tw, th)

                    raw = src.read([1, 2, 3], window=win).astype(np.float32) / 255.0
                    rgb = np.moveaxis(raw, 0, -1)

                    alpha_valid = (src.read(4, window=win) > 0
                                   if has_alpha
                                   else np.ones((th, tw), dtype=bool))

                    ft        = compute_features(rgb)
                    flat      = ft.reshape(-1, ft.shape[-1])
                    mask      = alpha_valid.ravel()
                    pred_flat = np.zeros(th * tw, dtype=np.uint8)

                    if mask.sum() > 0:
                        proba    = clf.predict_proba(flat[mask])
                        max_prob = proba.max(axis=1)
                        pred_cls = clf.classes_[proba.argmax(axis=1)].astype(np.uint8)
                        pred_cls[max_prob < CONF_THRESHOLD] = CLASSE_OUTROS_ID
                        pred_flat[mask] = pred_cls

                    dst.write(pred_flat.reshape(th, tw)[np.newaxis], window=win)

                    done += 1
                    pct  = 100 * done / total_tiles
                    bar  = "█" * int(pct / 5) + "░" * (20 - int(pct / 5))
                    print(f"\r  [{bar}] {pct:5.1f}%  ({done}/{total_tiles} tiles)",
                          end="", flush=True)

    print(f"\n  Salvo: {pred_path}")
    return pred_path


# ─────────────────────────────────────────────────────────────────────────────
# AGREGAÇÃO DE PEQUENOS POLÍGONOS / BURACOS
# ─────────────────────────────────────────────────────────────────────────────

def aggregate_small_regions(raster, min_px, struct):
    """
    Para cada componente conexo cujo tamanho < min_px:
      - Determina o label predominante na vizinhança (dilatação 3×3)
      - Substitui o componente por esse label vizinho
    Isso vale tanto para polígonos pequenos quanto para buracos internos.
    """
    raster = raster.copy()
    # Itera por todas as labels presentes (inclusive CLASSE_OUTROS_ID)
    all_ids = np.unique(raster)
    all_ids = all_ids[all_ids > 0]

    changed = True
    passes  = 0
    while changed and passes < 10:
        changed = False
        passes += 1
        for cid in all_ids:
            binary = (raster == cid)
            labeled, n_comp = nd_label(binary, structure=struct)
            sizes = np.bincount(labeled.ravel())   # tamanho de cada componente
            for comp_id in range(1, n_comp + 1):
                if sizes[comp_id] >= min_px:
                    continue
                # Componente pequeno: encontra vizinho dominante por dilatação
                comp_mask = (labeled == comp_id)
                dilated   = maximum_filter(
                    raster * ~comp_mask,    # label 0 dentro do componente
                    size=3,
                    mode="constant", cval=0
                )
                neighbor_labels = dilated[comp_mask]
                neighbor_labels = neighbor_labels[neighbor_labels > 0]
                if len(neighbor_labels) == 0:
                    continue
                dominant = int(np.bincount(neighbor_labels).argmax())
                if dominant == 0:
                    dominant = int(neighbor_labels[0])
                raster[comp_mask] = dominant
                changed = True

    return raster


# ─────────────────────────────────────────────────────────────────────────────
# ETAPA 3 — VETORIZAÇÃO
# ─────────────────────────────────────────────────────────────────────────────

def vectorize(pred_path):
    print("\n─" * 60)
    print("ETAPA 4: VETORIZAÇÃO + LIMPEZA MORFOLÓGICA")
    print("─" * 60)

    vec_dir = os.path.join(OUT_DIR, "vetores")
    os.makedirs(vec_dir, exist_ok=True)

    with rasterio.open(pred_path) as src:
        data      = src.read(1)
        transform = src.transform
        crs       = src.crs
        res_m     = abs(transform.a)

    px_area = res_m ** 2
    min_px  = max(1, int(MIN_AREA_M2 / px_area))

    print(f"  Resolução   : {res_m*100:.1f} cm/px")
    print(f"  Área mínima : {MIN_AREA_M2} m²  =  {min_px} px  (polígonos E buracos)")
    print()

    # ── Distribuição bruta ────────────────────────────────────────────────────
    print("  Distribuição das classes preditas (antes da limpeza):")
    total_px = int((data > 0).sum())
    all_class_ids = list(CLASSES.keys()) + [CLASSE_OUTROS_ID]
    for cid in all_class_ids:
        n   = int((data == cid).sum())
        pct = 100 * n / max(total_px, 1)
        print(f"    {CLASS_NAMES[cid]:12s}: {n:>14,} px  ({pct:5.1f}%)")
    print()

    # ── Limpeza morfológica global ────────────────────────────────────────────
    struct = np.ones((3, 3), dtype=bool)
    data_clean = data.copy()
    for cid in all_class_ids:
        mask = (data_clean == cid).astype(bool)
        if mask.sum() == 0:
            continue
        mask = binary_opening(mask, structure=struct, iterations=SMOOTH_ITER)
        mask = binary_closing(mask, structure=struct, iterations=SMOOTH_ITER)
        data_clean = np.where(mask, cid, data_clean)

    # ── Agrega polígonos pequenos e buracos ao vizinho dominante ─────────────
    print("  Agregando regiões menores que a área mínima ao vizinho dominante…")
    data_clean = aggregate_small_regions(data_clean, min_px, struct)
    print("  Agregação concluída ✓")
    print()

    # ── Vetoriza classe a classe ──────────────────────────────────────────────
    schema = {
        "geometry": "Polygon",
        "properties": {
            "class_id":   "int",
            "class_name": "str",
            "area_m2":    "float",
        },
    }

    # Decide quais classes exportar (as configuradas + outros se houver pixels)
    export_ids = list(CLASSES.keys())
    if (data_clean == CLASSE_OUTROS_ID).sum() > 0:
        export_ids.append(CLASSE_OUTROS_ID)

    for cid in export_ids:
        cname = CLASS_NAMES[cid]
        mask  = (data_clean == cid).astype(np.uint8)
        if mask.sum() == 0:
            print(f"  {cname}: sem pixels, pulando.")
            continue

        # Vetoriza
        polys = []
        for geom_dict, val in rio_shapes(mask, mask=mask, transform=transform):
            if int(val) != 1:
                continue
            p = shape(geom_dict)
            if p.area < MIN_AREA_M2:
                continue
            polys.append(p)

        if not polys:
            print(f"  {cname}: sem polígonos acima de {MIN_AREA_M2} m², pulando.")
            continue

        tol   = res_m * 0.5
        feats = []
        for p in polys:
            ps = p.simplify(tol, preserve_topology=True)
            if ps.is_empty:
                continue
            feats.append({
                "geometry":   mapping(ps),
                "properties": {
                    "class_id":   cid,
                    "class_name": cname,
                    "area_m2":    round(p.area, 3),
                },
            })

        if not feats:
            continue

        # GeoJSON
        geojson_path = os.path.join(vec_dir, f"classe_{cname}.geojson")
        with open(geojson_path, "w", encoding="utf-8") as fp:
            json.dump({
                "type": "FeatureCollection",
                "crs":  {"type": "name",
                          "properties": {"name": crs.to_string()}},
                "features": [{"type": "Feature", **f} for f in feats],
            }, fp, ensure_ascii=False)

        # Shapefile
        shp_path = os.path.join(vec_dir, f"classe_{cname}.shp")
        with fiona.open(shp_path, "w",
                        driver="ESRI Shapefile",
                        crs=crs.to_wkt(),
                        schema=schema) as dst:
            dst.writerecords(feats)

        total_ha = sum(f["properties"]["area_m2"] for f in feats) / 10_000
        print(f"  {cname:12s}: {len(feats):>6,} polígonos  |  {total_ha:.2f} ha  →  {shp_path}")

    print(f"\n  Todos os vetores em: {vec_dir}")


# ─────────────────────────────────────────────────────────────────────────────
# MAIN
# ─────────────────────────────────────────────────────────────────────────────

if __name__ == "__main__":
    image_path    = find_tiff()
    force_retrain = "--retrain" in sys.argv or not os.path.exists(MODEL_PATH)

    print("=" * 60)
    print("  CLASSIFICADOR DE USO DO SOLO — Random Forest")
    print("=" * 60)
    print(f"  Imagem : {image_path}")
    print(f"  Saída  : {OUT_DIR}")
    print(f"  Modelo : {'[novo treino]' if force_retrain else '[reutilizando modelo salvo]'}")
    print(f"  Classes: {', '.join(cfg['nome'] for cfg in CLASSES.values())}")
    print("=" * 60)

    if force_retrain:
        clf = train(image_path)
    else:
        print(f"\n  Carregando modelo: {MODEL_PATH}")
        print("  (use --retrain para forçar novo treino)")
        clf = joblib.load(MODEL_PATH)

    pred_path = predict(clf, image_path)
    vectorize(pred_path)

    print("\n" + "=" * 60)
    print("  PIPELINE CONCLUÍDO")
    print("=" * 60)
    print(f"  TIFF classificado  →  saida/classificado.tif")
    print(f"  Vetores por classe →  saida/vetores/classe_<nome>.shp")
    print(f"                        saida/vetores/classe_<nome>.geojson")
    print()
    print("  Dicas para o QGIS:")
    print("  • Abra o .tif com 'Estilo → Valores únicos' para visualizar")
    print("  • Os .shp já têm campo area_m2 para filtrar por tamanho")
    print("  • Para re-treinar: python main4.py --retrain")
    print("=" * 60)