                ...
                if mask_gen is not None:
                    ...
                    try:
                        ...
                        for mask_dict in masks_list:
                            ...
                            # [BUG-3] / [BUG-8] Alinha shape da máscara ao tile real (th, tw)
                            if mask_array.shape != (th, tw):
                                # Recorta se for maior, ou ignora se for menor demais
                                if mask_array.shape[0] < th or mask_array.shape[1] < tw:
                                    # Máscara menor que o tile — expansão não é segura, pula
                                    continue
                        ...
                        # Fallback: usa RF puro para este tile
...
def vectorize_final(final_raster_path, res_m, transform, crs):
    """
    [BUG-7] Verifica se o CRS é geográfico (graus) antes de comparar área em m².
    Em CRS geográfico, shapely.area retorna graus², não m² — aviso é emitido.
    """
    print("\n" + "─" * 60)
    ...
    if geo_crs:
        ...
        min_area = 0.0  # sem filtro de área em CRS geográfico
    else:
    ...
    # IDs de saída: 1, 2, 3 e 99
    all_ids = list(CLASSES.keys()) + [CLASSE_OUTROS_ID_SAIDA]
    for cid in all_ids:
