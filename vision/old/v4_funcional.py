...
SHAPES = {
    ...
    2: os.path.join(DATA_DIR, "solo.shp"),
    3: os.path.join(DATA_DIR, "floresta.shp"),
    # Para adicionar a classe "outros" com shapefile próprio:
    # 4: os.path.join(DATA_DIR, "outros.shp"),
}

CLASS_NAMES  = {1: "palhada", 2: "solo", 3: "floresta", 4: "outros"}
...
# Vetorização
MIN_AREA_M2       = 1.0      # polígonos menores que isso são descartados
...
def vectorize(pred_path):
    ...
    for cid, cname in CLASS_NAMES.items():
        ...
        print(f"  {cname}: {len(feats):>6,} polígonos  |  {total_ha:.2f} ha  →  {shp_path}")
