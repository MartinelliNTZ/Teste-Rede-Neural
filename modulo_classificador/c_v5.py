...
# Pasta onde este script está salvo — base para todos os caminhos relativos
...
CONFIG = {
    ...
    "path_img"            : _p("dados", "imagem.tif"),

    # Imagem a ser CLASSIFICADA (pode ser a mesma que path_img)
    "path_img_teste"      : _p("dados", "imagemFull.tif"),

    # Onde salvar o GeoTIFF classificado
    "path_saida"          : _p("resultado", "mapa_classificado_v8.tif"),
    ...
    # ── Máscara ───────────────────────────────────────────────────────────
    # True = filtra pixels pela última banda (canal alpha/máscara)
    "usar_mascara"        : True,
    # Valor da máscara que indica pixel válido (normalmente 255)
    "valor_valido"        : 255,

    # ── Recursos ──────────────────────────────────────────────────────────
    # Percentual máximo de RAM a usar, ex: 70 = 70% da RAM total
    "ram_limite_pct"      : 70,

    # ── Treino ────────────────────────────────────────────────────────────
    # Fração dos dados para teste, ex: 0.3 = 30%
    "test_size"           : 0.3,
    # Semente aleatória (qualquer inteiro; garante reprodutibilidade)
    "random_state"        : 42,
    # Máximo de épocas de treino
    "epochs"              : 100,
    # Batch do treinamento
    "batch_size_treino"   : 512,

    # ── Predição ──────────────────────────────────────────────────────────
    # Batch da predição (pode ser bem maior que o de treino)
    "batch_size_predicao" : 4096,

    # ── Arquitetura ───────────────────────────────────────────────────────
    # Neurônios por camada oculta
    "camadas_ocultas"     : [256, 128, 64, 8],
    # Função de ativação das camadas ocultas: relu, elu, tanh
    "activation"          : "relu",

    # ── Modelo ────────────────────────────────────────────────────────────
    # True = salva o modelo treinado em disco para reutilização futura
    "salvar_modelo"       : True,
    "path_modelo"         : _p("resultado", "modelo.keras"),
}
