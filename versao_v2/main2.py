...
CONFIG = {
    ...
    "path_saida": "resultado/mapa_classificado_v2_2.tif",
...
class ClassificadorRaster:
    ...
    def prever_imagem(self, batch_size_pred: int = 4096):
        """
        Classifica a imagem de teste (path_img_teste) em chunks de linhas.
        Pode ser uma imagem completamente diferente da usada no treino.
        NÃO normaliza — mesma escala que entrou no treino.
        """
        print(f"  Imagem a classificar: {self.path_img_teste}")

        with rasterio.open(str(self.path_img_teste)) as src:
            altura         = src.height
            largura        = src.width
        ...
        # Verifica compatibilidade de bandas entre treino e classificação
        with rasterio.open(str(self.path_img_teste)) as src_chk:
            n_feat_classif = src_chk.count - 1 if self.usar_mascara else src_chk.count
        if n_feat_classif != self.n_bandas_feature:
            raise ValueError(
                f"Incompatibilidade de bandas: treino usou {self.n_bandas_feature} bandas, "
                f"mas a imagem de classificacao tem {n_feat_classif} bandas de feature."
            )
        ...
        with rasterio.open(str(self.path_img_teste)) as src, \
            ...
            for i in range(n_chunks):
                ...
                validos = (mascara_2d.reshape(-1) == self.valor_valido
                           ...
                           else np.ones(feat_2d.shape[0], dtype=bool))
                ...
                if len(pixels_validos) > 0:
                    ...
                    pred_cls = (np.round(pred_raw).astype(float).flatten()
                                if self.num_classes == 2
                                else np.argmax(pred_raw, axis=1).astype(float))
